...
async function fetchLatestReleasesByStream(repo, stream) {
  ...
  const streamRelease = releases.find(release => {
    return release.tag_name.startsWith(`${stream}-`) || release.tag_name.startsWith(`${stream}.`);
  });
...
function parseChangelogVersions(body) {
  ...
  // Split by lines and find the Major packages section
  ...
  for (const line of lines) {
    ...
    // End of major packages section
    if (inMajorPackages && trimmed.startsWith('###') && !trimmed.includes('Major packages')) {
      break;
    }
    
    if (inMajorPackages && trimmed.startsWith('| **')) {
...
async function updateStreamVersions() {
  ...
  try {
    ...
    const stableRelease = await fetchLatestReleasesByStream(REPOS.main, 'stable');
    if (stableRelease) {
